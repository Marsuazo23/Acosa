...
class Products extends Table {
    ...
    public static function getProductById(int $productId) {
      $sqlstr = "SELECT p.productId, p.productName, p.productDescription, p.productPrice AS originalPrice, p.productImgUrl, p.productStatus, p.stock, ROUND(p.productPrice - (p.productPrice * s.discountPercent / 100), 2) AS productPrice, CONCAT('-', CAST(s.discountPercent AS UNSIGNED), '%') AS discount FROM products p LEFT JOIN sales s ON p.productId = s.productId AND s.saleStart <= NOW() AND s.saleEnd >= NOW() WHERE p.productId = :productId AND p.productStatus = 'ACT'";
