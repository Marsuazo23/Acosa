...
class Products extends Table {
    ...
    $sqlstr = "SELECT p.productId, p.productName, p.productDescription, p.productPrice, p.productImgUrl, p.productStatus, c.categoryName FROM products p INNER JOIN categories c ON p.categoryId = c.categoryId WHERE p.productStatus = 'ACT' AND p.productId NOT IN (SELECT productId FROM sales WHERE saleStart <= NOW() AND saleEnd >= NOW()) AND p.productId = (SELECT MIN(p2.productId) FROM products p2 WHERE p2.categoryId = p.categoryId AND p2.productStatus = 'ACT' AND p2.productId NOT IN (SELECT productId FROM sales WHERE saleStart <= NOW() AND saleEnd >= NOW() ) ) LIMIT 4";
    $params = [];
    $registros = self::obtenerRegistros($sqlstr, $params);
    return $registros;
}
    ...
    $sqlstr = "SELECT p.productId, p.productName, p.productDescription, p.productPrice AS originalPrice, p.productImgUrl, p.productStatus, p.stock, 
               ...
               FROM products p
               LEFT JOIN sales s ON p.productId = s.productId AND s.saleStart <= NOW() AND s.saleEnd >= NOW()
